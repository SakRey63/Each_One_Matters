...
public class SpawnerSegmentBridge : MonoBehaviour
{
    ...
    public void CreateSegmentBridge(Vector3 positionSegment, BridgeObjectSelector objectSelector, bool isHorizontal, Quaternion targetRotation, Vector3 randomPositionSection)
    {
        ...
        {
            Instantiate(_damagedBridge, positionSegment, targetRotation);
            objectSelector.ResetAllStats();
        }
        else
        {
            Instantiate(_normalBrige, positionSegment, targetRotation);
        }
    }
