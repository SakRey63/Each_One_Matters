...
namespace EachOneMatters.Generation.PositionGeneration
{
    public class SegmentPositionGenerator : MonoBehaviour
    {
        ...
        public Vector3 GetPositionToBaseOfConnector(Vector3 nextPosition, float offset)
        {
            return _currentDirection switch
            {
                BridgeDirection.HorizontalRight => new Vector3(nextPosition.x + offset, _startPositionAllTriggers.position.y, nextPosition.z - _sectionOffset),
                BridgeDirection.HorizontalLeft => new Vector3(nextPosition.x - offset, _startPositionAllTriggers.position.y, nextPosition.z + _sectionOffset),
                BridgeDirection.VerticalUp => new Vector3(nextPosition.x + _sectionOffset, _startPositionAllTriggers.position.y, nextPosition.z + offset),
                _ => nextPosition
            };
        }
        ...
        public Vector3 GetObstaclePosition(Vector3 basePosition)
        {
            return _currentDirection switch
            {
                BridgeDirection.HorizontalRight => new Vector3(basePosition.x, basePosition.y, basePosition.z + (_obstacleSide == ObstacleSide.Right ? _monsterRightOffset : -_monsterLeftOffset)),
                BridgeDirection.HorizontalLeft => new Vector3(basePosition.x, basePosition.y, basePosition.z + (_obstacleSide == ObstacleSide.Right ? -_monsterRightOffset : _monsterLeftOffset)),
                BridgeDirection.VerticalUp => new Vector3(basePosition.x + (_obstacleSide == ObstacleSide.Right ? -_monsterRightOffset : _monsterLeftOffset), basePosition.y, basePosition.z),
                _ => basePosition
            };
        }
        ...
        {
            _lastHorizontalRotation = _lastHorizontalRotation == BridgeDirection.HorizontalRight ? BridgeDirection.HorizontalLeft : BridgeDirection.HorizontalRight;
        }
