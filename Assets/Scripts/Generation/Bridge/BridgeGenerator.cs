...
namespace EachOneMatters.Generation.Bridge
{
    public class BridgeGenerator : MonoBehaviour
    {
        ...
        private void CreateConnectorOrFinish(Vector3 nextSpawnPosition, int index)
        {
            ...
            {
                Vector3 position =
                    _positionGenerator.GetPositionToBaseOfConnector(nextSpawnPosition, _bridgeConnectorOffset);
        ...
        private void SpawnPowerUp(Vector3 basePosition)
        {
            switch (_objectSelector.CurrentType)
            {
                case BridgeObjectType.RecruitPolice:
                    RecruitPolice recruitPolice =
                        _buffSpawner.GetRecruitPolice(_positionGenerator.GetNextPositionAlongWidth(basePosition), _targetRotation);
